// -*- mode: c++ ; -*-
/** \file falaise/snemo/processing/test_calorimeter_s2c_module.cc
 */

// Ourselves:
#include "test_calorimeter_s2c_module.h"
...
namespace snemo {
  ...
  namespace processing {
    ...
    DPP_MODULE_REGISTRATION_IMPLEMENT(test_calorimeter_s2c_module,
                                      "snemo::processing::test_calorimeter_s2c_module");

    void test_calorimeter_s2c_module::set_geom_manager(const geomtools::manager & gmgr_)
    {
    ...
    const geomtools::manager & test_calorimeter_s2c_module::get_geom_manager() const
    {
      DT_THROW_IF(! is_initialized(),
                  std::logic_error,
    ...
    bool test_calorimeter_s2c_module::has_external_random() const
    {
      return _external_random_ != 0;
    }

    void test_calorimeter_s2c_module::reset_external_random()
    {
      DT_THROW_IF(is_initialized(),
                  std::logic_error,
                  "Module '" << get_name() << "' is already initialized ! ");
    ...
    void test_calorimeter_s2c_module::set_external_random(mygsl::rng & rng_)
    {
      DT_THROW_IF(is_initialized(),
                  std::logic_error,
                  "Module '" << get_name() << "' is already initialized ! ");
    ...
    void test_calorimeter_s2c_module::initialize(const datatools::properties  & setup_,
                                                 datatools::service_manager   & service_manager_,
                                                 dpp::module_handle_dict_type & /* module_dict_ */)
    {
      DT_LOG_TRACE(get_logging_priority(), "Enetring...");
      DT_THROW_IF(is_initialized(),
                  std::logic_error,
                  "Module '" << get_name() << "' is already initialized ! ");
      ...
      // Fetching mock database file
      DT_THROW_IF(! setup_.has_key("calibration_database_path"), std::logic_error,
                  "Missing 'calibration_database_path' property !");
      std::string db_path = setup_.fetch_string("calibration_database_path");
      datatools::fetch_path_with_env(db_path);
      DT_LOG_DEBUG(get_logging_priority(), "DB path = " << db_path);

      _parse_database_file(db_path);
    ...
    void test_calorimeter_s2c_module::reset()
    {
    ...
    void test_calorimeter_s2c_module::_set_defaults()
    {
      _external_random_ = 0;
      _geom_manager_ = 0;
      _SD_label_.clear();
    ...
    // Constructor :
    test_calorimeter_s2c_module::test_calorimeter_s2c_module(datatools::logger::priority logging_priority_)
    ...
    // Destructor :
    test_calorimeter_s2c_module::~test_calorimeter_s2c_module()
    {
      if (is_initialized()) test_calorimeter_s2c_module::reset();
      return;
    }

    // Processing :
    dpp::base_module::process_status test_calorimeter_s2c_module::process(datatools::things & event_record_)
    {
      DT_LOG_TRACE(get_logging_priority(), "Entering...");
      DT_THROW_IF(! is_initialized(),
                  std::logic_error,
    ...
    mygsl::rng & test_calorimeter_s2c_module::_get_random()
    {
    ...
    void test_calorimeter_s2c_module::_process_calorimeter_digitization
      ...
      for (std::vector<std::string>::const_iterator icategory = _hit_categories_.begin();
        ...
        for (size_t ihit = 0; ihit < nb_hits; ++ihit) {
          ...
          if (a_calo_mc_hit.get_particle_name() == "alpha" && _alpha_quenching_) {
            DT_THROW_IF(_calorimeter_regimes_.find(gid) == _calorimeter_regimes_.end(),
                        std::logic_error,
                        "Missing calorimeter regime for '" << gid << "'!");
            const calorimeter_regime & the_calo_regime = _calorimeter_regimes_.at(gid);
    ...
    void test_calorimeter_s2c_module::_process_calorimeter_calibration
    (snemo::datamodel::calibrated_data::calorimeter_hit_collection_type & calibrated_calorimeter_hits_)
    {
      DT_LOG_DEBUG(get_logging_priority(), "Entering..." );
      ...
      for (snemo::datamodel::calibrated_data::calorimeter_hit_collection_type::iterator
        ...
        // first recover the calorimeter regime associated to geom_id
        // const std::string & category_name
        //   = the_calo_cluster.get_auxiliaries().fetch_string("category");
        const geomtools::geom_id & a_gid = the_calo_cluster.get_geom_id();
        DT_THROW_IF(_calorimeter_regimes_.find(a_gid) == _calorimeter_regimes_.end(),
                    std::logic_error,
                    "Missing calorimeter regime for '" << a_gid << "'!");
        const calorimeter_regime & the_calo_regime = _calorimeter_regimes_.at(a_gid);
    ...
    void test_calorimeter_s2c_module::_process_calorimeter_trigger
    (snemo::datamodel::calibrated_data::calorimeter_hit_collection_type & calibrated_calorimeter_hits_)
    {
      DT_LOG_DEBUG(get_logging_priority(), "Entering..." );
    ...
    void test_calorimeter_s2c_module::_process
    (const mctools::simulated_data & simulated_data_,
     snemo::datamodel::calibrated_data::calorimeter_hit_collection_type & calibrated_calorimeter_hits_)
    {
      DT_LOG_DEBUG(get_logging_priority(), "Entering...");
    ...
    void test_calorimeter_s2c_module::_parse_database_file(const std::string & db_path_)
    {
      ...
      while (std::getline(db, line)) {
        ...
        oss << '['
            ...
            << cells[3] << '.' << cells[4]
            << ']';
        DT_LOG_DEBUG(get_logging_priority(), "oss = " << oss.str());
        std::istringstream iss(oss.str());
        geomtools::geom_id a_gid;
        iss >> a_gid;
        // Make sure the geom_id is syntaxically valid
        DT_THROW_IF(! a_gid.is_valid(), std::logic_error,
                    "Geom id syntax from '" << oss.str() << "' is not valid !");
        // Special case for main wall and the part adress
        if (a_gid.get_type() == 1302) {
          a_gid.set_any(a_gid.get_depth());
        }
        DT_LOG_DEBUG(get_logging_priority(), "geom_id = " << a_gid);
        // Make sure the geom_id is inside the current detector
        const geomtools::manager & gmgr = *_geom_manager_;
        const geomtools::id_mgr & id_mgr = gmgr.get_id_mgr();
        // const geomtools::mapping & mapping = gmgr.get_mapping();
        // DT_THROW_IF(! mapping.validate_id(a_gid), std::logic_error,
        //             "Geom id " << a_gid << "' is not geometrically valid !");
        // Create the calorimeter regime associated to the geom_id
        {
          snemo::processing::calorimeter_regime dummy;
          _calorimeter_regimes_[a_gid] = dummy;
        }
        calorimeter_regime & a_regime = _calorimeter_regimes_[a_gid];
        datatools::properties config;
        config.update_with_explicit_unit("energy.resolution",
                                         datatools::units::get_value_with_unit(cells[5]));
        a_regime.initialize(config);
        if (get_logging_priority() >= datatools::logger::PRIO_DEBUG) {
          DT_LOG_DEBUG(get_logging_priority(), "Calorimeter regime for '" << a_gid << "':");
          a_regime.tree_dump(std::clog, "", "[trace]: ");
        }
      }
      return;
    }
 ...
 *  @arg snemo::processing::test_calorimeter_s2c_module the full class name
 ...
 */
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(snemo::processing::test_calorimeter_s2c_module,ocd_)
{
  ocd_.set_class_name("snemo::processing::test_calorimeter_s2c_module");
  ocd_.set_class_description("A module that performs a test digitization/calibration of the calorimeter simulated data");
...
// Registration macro for class 'snemo::processing::test_calorimeter_s2c_module' :
DOCD_CLASS_SYSTEM_REGISTRATION(snemo::processing::test_calorimeter_s2c_module,
                               "snemo::processing::test_calorimeter_s2c_module")

// end of falaise/snemo/processing/test_calorimeter_s2c_module.cc
